...
from pydglab_ws import StrengthData, Channel, StrengthOperationType, RetCode, DGLabWSServer, FeedbackButton
...
def upload_qrcode(qq_id: str):
    """调用sm.ms的api来储存二维码图片"""
    # 返回信息是一个下载的url
    # 例 http://example.com/qrcode.png
    return "http://ricedev.top:803/qrcode.png"
    #raise NotImplementedError("请自行实现上传二维码图片功能")
class Commander:
    ...
    async def __send_pulse(self):
        while True:
            if self.pulse_close_tag:
                ...
                return
            if self.client:
                max_chunk = getattr(self.client, "max_pulse_chunk", len(self.current_pulses_A))
                pulses_A_data = self.current_pulses_A
                pulses_B_data = self.current_pulses_B
                pulses_A = []
                pulses_B = []
                for i in range(max_chunk):
                    pulses_A.append(pulses_A_data[i % len(pulses_A_data)])
                for i in range(max_chunk):
                    pulses_B.append(pulses_B_data[i % len(pulses_B_data)])
                await self.client.clear_pulses(Channel.A)
                await self.client.clear_pulses(Channel.B)
                await self.client.add_pulses(Channel.A, *pulses_A)
                await self.client.add_pulses(Channel.B, *pulses_B)
    ...
    async def random_dice(self):
        if self.size == 1:
            # 没有参数，返回1-6之间的随机整数
            result = random.randint(1, 6)
            await self.send_message(f"🎲 掷骰子结果：{result}")
        elif self.size == 2:
            # 有一个参数，检查是否为有效数字
            if not self.kwargs[0].isdigit():
                await self.send_message("参数必须是正整数")
                return
            
            max_value = int(self.kwargs[0])
            if max_value <= 0:
                await self.send_message("参数必须是正整数")
                return
            
            result = random.randint(1, max_value)
            await self.send_message(f"🎲 掷点范围(1-{max_value})：掷点结果{result}")
        else:
            await self.send_message("掷骰子命令格式错误，应为'/掷点'或'/掷点 数字'")
            _log.warning(f'用户{self.qq_id}掷骰子命令参数过多：{self.size - 1}')
    ...
    async def help(self):
        await self.send_message('这里是命令介绍喵~\r\n\r\n'
                                ...
                                '「掷点」命令用于掷骰子, 如: /掷点 或 /掷点 20\r\n'
                                '「设置名称」命令用于获取当前用户的ID,获取后联系呱呱糕添加进去\r\n'
                                '「帮助」命令用于查看所有命令\r\n'
                                )
